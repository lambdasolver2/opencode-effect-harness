...
import { Decision } from './enforcement/Decision.ts';
import { Intent } from './enforcement/Intent.ts';
import { Projection } from './enforcement/Projection.ts';
import { Gate as GateRule } from './enforcement/rules/Gate.ts';
import { Header as HeaderRule } from './enforcement/rules/Header.ts';
import { Feedback as FeedbackRule } from './enforcement/rules/Feedback.ts';

import { typescriptModule } from './verification/modules/Typescript.ts';
import { Registry } from './verification/Module.ts';
import { skillEntriesFromAssets } from './verification/Module.ts';
import { Orchestrator } from './verification/Orchestrator.ts';
import { VerifierReport, VerifyRequest } from './verification/Report.ts';

import { Journal } from './shared/Journal.ts';
import { projectKeyOf } from './shared/Refs.ts';

import { decode, defaults as defaultOptions } from './opencode/Options.ts';
import { ExecNode } from './opencode/ExecNode.ts';
import { Sessions } from './opencode/Sessions.ts';
import { Origins } from './opencode/Origins.ts';
import { ModeState } from './opencode/ModeState.ts';
import { Ledger, PendingReads, type HostStorage } from './opencode/Ledger.ts';
import { ChangeLedger } from './opencode/ChangeLedger.ts';
import * as Events from './opencode/Events.ts';
import { LiveTraceSink } from './opencode/Events.ts';
import * as CapabilityModule from './opencode/Capability.ts';
	...
	effect: (ctx) =>
		Effect.gen(function* () {
			...
			const assetsRoot =
				...
				new URL('../effect-harness/', import.meta.url).pathname.replace(/\/$/, '');
			...
			const tsModule = yield* Effect.orElseSucceed(
				providePlatform(typescriptModule(assetsRoot)),
				() => undefined
			);
			const registry = Registry.make(tsModule === undefined ? [] : [tsModule]);
...
const loadPatternsSafe = (
	assetsRoot: string
): Effect.Effect<ReadonlyArray<import('./enforcement/Pattern.ts').Pattern.Value>> =>
	Effect.flatMap(
		Effect.promise(() => import('./enforcement/Catalog.ts')),
		(catalog) =>
