 ...
 *
 * Justified entries:
 *  - src/Path.ts — SECURITY adapter (AUDIT-035): symlink-hardened
 ...
 *    design.
 *  - prefer-option-over-null on benchmark schema modules (Scorecard/TaskStore/
 *    Evaluator/Benchmark Runner+Tool/Session Executor+Sessions+Origins/SqliteStore) —
 *    Schema `optionalKey` fields and SQLite NULL columns encode absence as
 *    undefined/NULL at the persistence boundary by design; domain logic
 *    itself uses Option.
 *  - prefer-schema-class on Evaluator/Benchmark Runner+Tool/
 *    SqliteStore — anonymous inline Structs for small wire payloads and
 *    SQLite row codecs; identity classes reserved for domain entities
 *    (same disposition as verify-kit/report.ts).
 *  - casting-awareness on SqliteStore — the single widening cast at the
 *    store layer export (driver/migration error union → port error union);
 *    all row data flows through Schema codecs.
 */
export const baseline: Readonly<Record<string, ReadonlyArray<string>>> = {
	...
	'packages/compound-kit/src/Evaluator.ts': ['prefer-option-over-null', 'prefer-schema-class'],
	'packages/compound-kit/src/Scorecard.ts': ['prefer-option-over-null'],
	'packages/compound-kit/src/task/Store.ts': ['prefer-option-over-null'],
	...
	'packages/harness-kit/src/Controller.ts': ['casting-awareness', 'prefer-option-over-null'],
	'packages/harness-kit/src/Edit.ts': ['prefer-option-over-null'],
	'packages/harness-kit/src/Engine.ts': ['casting-awareness'],
	...
	'packages/module-bend/src/index.ts': ['casting-awareness', 'prefer-option-over-null'],
	...
	'packages/shared/src/Model.ts': ['avoid-untagged-errors'],
	'packages/shared/src/path/Guard.ts': ['prefer-option-over-null'],
	'packages/verify-kit/src/Checker.ts': ['prefer-option-over-null'],
	...
	'src/session/Live.ts': ['casting-awareness', 'prefer-effect-fn', 'prefer-option-over-null', 'use-clock-service'],
	'src/mode/State.ts': ['casting-awareness', 'effect-run-in-body', 'prefer-option-over-null', 'use-clock-service'],
	'src/Options.ts': ['prefer-option-over-null', 'prefer-schema-class'],
	'src/session/Origin.ts': ['effect-run-in-body', 'prefer-option-over-null', 'require-effect-concurrency'],
	'src/session/Session.ts': ['casting-awareness', 'effect-run-in-body', 'prefer-option-over-null'],
	...
	'src/companion/Cli.ts': ['avoid-direct-json', 'avoid-process-env', 'casting-awareness', 'effect-run-in-body'],
	...
	'src/Path.ts': ['prefer-option-over-null', 'use-filesystem-service'],
	'src/benchmark/Runner.ts': ['prefer-option-over-null', 'prefer-schema-class'],
