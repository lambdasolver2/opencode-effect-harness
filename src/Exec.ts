...
const spawnOnce = (
	...
	Effect.flatMap(
		Effect.callback<SpawnOutcome, ExecError>((resume) => {
			...
			const finish = (code: number | null, signal: NodeJS.Signals | null) => {
				...
				settled = true;
				clearTimeout(timer);
				resume(Effect.succeed({ code, signal, stdout: out, stderr: err, timedOut: didTimeOut, truncated }));
			};
			...
			;(
				...
				settled = true;
				clearTimeout(timer);
				resume(Effect.fail(new ExecError({ reason: 'spawn failed', command: spec.executable })));
				void cause;
			});
			;(
				child as unknown as {
			...
			return Effect.sync(() => {
				...
				if (!settled) {
					...
					resume(Effect.fail(new ExecError({ reason: 'interrupted', command: spec.executable })));
				}
			});
		}),
		(outcome) =>
